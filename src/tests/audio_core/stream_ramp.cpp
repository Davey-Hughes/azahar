...
TEST_CASE("StreamRamp tail continues the waveform and decays to silence", "[audio_core][speedup]") {
...
TEST_CASE("StreamRamp ramps a resumed stream in from silence", "[audio_core][speedup]") {
    AudioCore::StreamRamp ramp;
    ramp.Begin();
...
TEST_CASE("StreamRamp keeps ramping in across a short first buffer", "[audio_core][speedup]") {
    AudioCore::StreamRamp ramp;
    REQUIRE(ramp.Down());
...
TEST_CASE("StreamRamp does not end a stream that was already silent", "[audio_core][speedup]") {
    AudioCore::StreamRamp ramp;
...
TEST_CASE("StreamRamp mutes what follows a tail before ramping back in", "[audio_core][speedup]") {
    AudioCore::StreamRamp ramp;
    PlayTone(ramp);
    ramp.End();
